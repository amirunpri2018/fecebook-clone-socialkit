...
if ($t == "message") {
    ...
    if ($a == "delete") {
        ...
        if (!empty($_POST['message_id']) && is_numeric($_POST['message_id']) && $_POST['message_id'] > 0) {
            ...
            $query_one = "SELECT id FROM " . DB_POSTS . " WHERE id=$message_id AND timeline_id=" . $user['id'] . " AND type1='message' AND active=1";
            $sql_query_one = mysqli_query($dbConnect, $query_one);
            
            if (mysqli_num_rows($sql_query_one) == 1) {
                $query_two = "DELETE FROM " . DB_POSTS . " WHERE id=$message_id AND timeline_id=" . $user['id'] . " AND type1='message' AND active=1";
                $sql_query_two = mysqli_query($dbConnect, $query_two);
...
if ($t == "cover") {
    ...
    if ($a == "new" && isset($_FILES['image']['tmp_name'])) {
        ...
        if ($continue == true) {
            $cover_data = SK_registerMedia($cover_image);
            ...
            if (isset($cover_data['id'])) {
                ...
                if ($sql_query) {
                    $mobile_cover_data = SK_createCover($cover_data['id'], 0);
                    
                    if ($mobile_cover_data) {
                        $data = array(
                            'status' => 200,
                            'cover_url' => $sk['config']['site_url'] . '/' . $mobile_cover_data,
